...
- `GhwtdeIniTool/src-tauri/tauri.conf.json` configures the desktop shell,
  including the canonical release version, main window, bundling, and updater
  artifacts.
...
- `pnpm tauri` runs the Tauri CLI through `scripts/tauri.ts`, which generates
  updater endpoint configuration. The `tauri.conf.json` version is retained for
  builds; `GHWTDE_APP_VERSION` can override it only for `pnpm tauri dev` updater
  testing.
...
- `GhwtdeIniTool/src-tauri/tauri.conf.json` defines app metadata, the canonical
  release version used locally and by GitHub Actions, windows, bundling, updater
  settings, and frontend build integration.
