...
- `GhwtdeIniTool/src-tauri/about.toml` and `about.hbs` configure the
  production Rust dependency notices generated by `cargo-about`.
...
- `src-tauri/resources/licenses/NewRocker-OFL-1.1.txt` is the maintained source
  ...
  Third-party licenses dialog. Run `pnpm licenses:generate` before a release
  to recreate it from the font notice and production frontend/Rust dependency
  notices.
